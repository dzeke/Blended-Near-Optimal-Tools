...
The AmmanJordanApplication folder contains the data, model, and script files used for the mixed-integer water supply/demand management application for Amman, Jordan that illustrates use of the integer enumeration, visualization, and interaction tools. On the interactive parallel coordinate plot visualization tool, controls within the Interact Tab=>Generate New Alternatives box require a GAMS license to run (generation is done in GAMS; contact GAMS at http://gams.com). Other tools on Color Ramp and Display tabs can run without a GAMS license (run within Matlab). This application is presented in:
...
Download all the files in this folder as well as in the InteractiveParallelPlot folder. Use the files in this folder as follows:
