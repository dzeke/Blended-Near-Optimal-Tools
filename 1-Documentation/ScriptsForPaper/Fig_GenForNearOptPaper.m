...
% four linear constraints, tolerable deviation of ?=1.80, and generation density of 0.5 alternatives per decision 
...
%           >> [mResult nO vParams] = NearOptimalLP2('WQNE_outG6.gdx',3,2500,0,[2 0 50]);
...
%   4. From the Controls Menu=>Sliders=>uncheck Hide sliders (show them).
%       Sliders will appear on the plot all set to minimum allowable values
%   5. Set the slider for the Phos. Removed axis to it's upper extent.
...
%        - Similarly, change the order for the MGA group to 2 to 3
%        - Click the Reorder Groups button
...
%   1. Generate Figure 3. At the commnad prompt paste:
%          >> [mResult nO vParams] = LoadEchoGamsResultsMGAComp('WQNE_outG6.gdx',3,2500,0,[3 0 10]);
%   2. Once the figure finally loads, click the Interact tab
...
%        2ide it)
%        - Uncheck the MGA group (to hide it)
%        - Click the Reorder Groups button and the Tolerance 1.25 group
%        will plot behind the prior Near-optimal 1.10 group and it is possible to see
...
bReply = inputdlg(['Finished auto-generating Figures 1, 2, 3, and 5. To interactively create Figures 4, 6, and 7 from Figures 3 and 5, ' ...
...
if strcmpi(bReply,'Yes')
    ...
    %      b. For the HSJ results from the current model, to see the number of decision variables below 100 kg phosphorus,
    %         enter the command:
    %           >> sum(max(no_mDecs(strcmpi(no_vargs{26},'MGA-HSJ'),:))' < 100)
    %      c. To see the number of decision variables below the extents
    %         identified by stratified sampling, enter the commend:
    %           >> sum(max(no_mDecs(strcmpi(no_vargs{26},'MGA-HSJ'),:))' < max(no_mDecs(strcmpi(no_vargs{26},'Near-optimal'),:))')
    %      d. To see which variables are below 100 kg, enter the commands:
    %           >> actFull = [ no_vargs{24} no_vargs{48}' ]
    %           >> actFull(max(no_mDecs(strcmpi(no_vargs{26},'MGA-HSJ'),:))' < 100,:)
    %      e. To see similar results for GAMS model with land area and stream
    %         bank lenght decision variables, sbustitute 'MGA-GAMS-HSJ' for
    %         'MGA-HSJ', e.g.
    %           >> sum(max(no_mDecs(strcmpi(no_vargs{26},'MGA-GAMS-HSJ'),:))' < 100)     
